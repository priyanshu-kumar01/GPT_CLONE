...
const Sidebar = () => {
  ...
  return (
    ...
    border-4 border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1'>
      ...
      {/* New chat button */}
      <button className='flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-purple-600 to-blue-800 text-sm rounded-md cursor-pointer'>
        <span className='mr-2 text-xl'>+</span> New Chat
      </button>

      {/* Search */}
      <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
        <img src={assets.search_icon} className='w-4 dark:invert-0' alt="search" />
        <input 
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        ...
        <div onClick={()=> {navigate('/credits')}} className='flex flex-items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all' >
          ...
          <div className='flex flex-col text-sm'>
            ...
            <p className='text-xs text-grey-400'>Purchase credits to use QuickGPT</p>
          </div>
        </div>
       ...
       {/* Dark Mode Toggle */}
        ...
        {/* User Account */}
        <div className='flex flex-items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group' >
